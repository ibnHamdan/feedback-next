...
export async function getAllFeedback(siteId) {
  try {
    const feedback= []
  const snapshot =await admin.firestore().collection('feedback').where('siteId', '==', siteId).get();
...
export async function getUserFeedback(uid){
  ...
  try{
    const feedback= []
    const snapshot =await admin.firestore().collection('feedback').where('authorId', '==', uid).get();
    snapshot.forEach((doc) => {
      feedback.push({id: doc.id, ...doc.data()})
  });
